...
from app.schemas.dashboard_schemas import DashboardFilterParamaters, TimeFilterParameter
router = APIRouter()
Dashboard_Service = DashboardService()

@router.post("/api/v1/dashboard/ilt/{iltId}")
async def Show_Ilt_Dashboard( iltId :int, FilterParamaters:TimeFilterParameter, UserId: int=Header(convert_underscores=False), db: Session = Depends(get_db)):
    return Dashboard_Service.get_ilt_Meetings_dashboard_info(user_id=UserId, 
                                                             FilterParamaters=FilterParamaters , ilt_id=iltId, db=db)

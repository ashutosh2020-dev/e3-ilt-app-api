...
def fn_create_user(UserId: int, user:UserRequest, db: Session = Depends(get_db)):
...
def fn_update_user(user_id: int, id:int, user:UserRequest, db: Session = Depends(get_db)):
    return user_service.update_user(user_id = user_id, 
                                    id = id, 
                                    fname=user.firstName, 
                                    lname=user.lastName,
                                    email=user.emailId, 

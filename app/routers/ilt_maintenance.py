...
async def create_ilt(user_id:int, Createilt:Createilt,  db: Session = Depends(get_db)):
...
def update_ilt(ilt_data:Ilt, id:int, user_id:int, db: Session = Depends(get_db)):
    return  IltService.update_ilt(ilt_data=ilt_data, user_id =user_id, ilt_id=id, db=db)

...
class DashboardService:
    ...
    def get_ilt_dashboard_info(user_id:int, ilt_id :int, db: Session):
        #check user 
        ...
        for mid in all_meeting_id_list:
            select_mid = (db.query(MdlMeetings)
                            .filter(MdlMeetings.id==mid, 
                                    MdlMeetings.end_at.replace(tzinfo=timezone.utc)<current_time)
                            .one_or_none())
            ...
            ## issue
                ...
                meetinns_issue_resolve_list = [db.query(Mdl_issue)
                                        .filter(Mdl_issue.id==issue_id)
                                        .one()
                                        .resolves_flag
                                        for issue_id in issue_id_list]
                issue_nominator = sum(meetinns_issue_resolve_list)
                issue_denominator = len(meetinns_issue_resolve_list)
                avg_issueResolve_list.append(issue_nominator/issue_denominator)
                

        aggregate_attandence = sum(avg_attandance_list)/len(avg_attandance_list)
        aggregate_rock = sum(avg_rock_list)/len(avg_rock_list)
        avg_rating = sum(avg_rating_list)/len(avg_rating_list) #take avg
        avg_issueResolve = sum(avg_issueResolve_list)/len(avg_issueResolve_list)
        avg_distribution_issue = sum(avg_issueResolve_list)/len(avg_issueResolve_list)
